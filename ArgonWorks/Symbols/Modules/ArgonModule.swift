...
public class ArgonModule: SystemModule
    ...
    private func initBaseMethods()
        ...
        mathGroup.addSymbol(IntrinsicMethodInstance(left:self.float,"truncate",right:self.float,out:self.integer).libraryMethod)
        mathGroup.addSymbol(IntrinsicMethodInstance(left:self.float,"ceiling",right:self.float,out:self.float).libraryMethod)
        mathGroup.addSymbol(IntrinsicMethodInstance(left:self.float,"floor",right:self.float,out:self.float).libraryMethod)
        mathGroup.addSymbol(IntrinsicMethodInstance(left:self.float,"round",right:self.float,out:self.float).libraryMethod)
        mathGroup.addSymbol(IntrinsicMethodInstance("sin",arg:self.float,out:self.float).libraryMethod)
        mathGroup.addSymbol(IntrinsicMethodInstance("cos",arg:self.float,out:self.float).libraryMethod)
        mathGroup.addSymbol(IntrinsicMethodInstance("tan",arg:self.float,out:self.float).libraryMethod)
        mathGroup.addSymbol(IntrinsicMethodInstance("asin",arg:self.float,out:self.float).libraryMethod)
        mathGroup.addSymbol(IntrinsicMethodInstance("acos",arg:self.float,out:self.float).libraryMethod)
        mathGroup.addSymbol(IntrinsicMethodInstance("atan",arg:self.float,out:self.float).libraryMethod)
        mathGroup.addSymbol(IntrinsicMethodInstance("ln",arg:self.float,out:self.float).libraryMethod)
        mathGroup.addSymbol(IntrinsicMethodInstance("exp",arg:self.float,out:self.float).libraryMethod)
        let formatGroup = SystemSymbolGroup(label: "Printing")
        self.addSymbol(formatGroup)
        let method = IntrinsicMethodInstance("print",self.integer).libraryMethod
        formatGroup.addSymbol(method)
        method.addInstance(IntrinsicMethodInstance("print",self.float))
        method.addInstance(IntrinsicMethodInstance("print",self.string))
        method.addInstance(IntrinsicMethodInstance("print",self.boolean))
        method.addInstance(IntrinsicMethodInstance("print",self.object))
        let ioGroup = SystemSymbolGroup(label: "IO")
        self.addSymbol(ioGroup)
        ioGroup.addSymbol(IntrinsicMethodInstance("next",self.readStream,"TYPE",self.integer).where("TYPE",self.object).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextPut",self.writeStream,"TYPE",self.integer).where("TYPE",self.object).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("open",self.string,self.string,"TYPE").where("TYPE",self.readStream).where("TYPE",self.writeStream).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("close",self.stream,self.boolean).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("tell",self.stream,self.integer).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("flush",self.stream,self.boolean).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("seek",self.stream,self.integer,self.boolean).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextLine",self.stream,self.string,self.void).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextPutLine",self.stream,self.string,self.void).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextByte",self.stream,self.byte,self.void).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextPutByte",self.stream,self.byte,self.void).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextFloat",self.stream,self.float).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextPutFloat",self.stream,self.float,self.void).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextInteger",self.stream,self.integer).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextPutInteger",self.stream,self.integer,self.void).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextString",self.stream,self.string).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextPutString",self.stream,self.string,self.void).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextSymbol",self.stream,self.symbol).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextPutSymbol",self.stream,self.symbol,self.void).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextUInteger",self.stream,self.uInteger).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextPutUInteger",self.stream,self.uInteger,self.void).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextDate",self.stream,self.date).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextPutDate",self.stream,self.date,self.void).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextTime",self.stream,self.time).libraryMethod)
        ioGroup.addSymbol(IntrinsicMethodInstance("nextPutTime",self.stream,self.time,self.void).libraryMethod)
        let arrays = SystemSymbolGroup(label: "Arrays")
        self.addSymbol(arrays)
        arrays.addSymbol(IntrinsicMethodInstance("append",self.array,"TYPE").libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("at",self.array,self.integer,"TYPE").libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("atPut",self.array,self.integer,"TYPE",self.void).libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("atPutAll",self.array,self.integer,self.array,self.void).libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("contains",self.array,"TYPE",self.boolean).libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("containsAll",self.array,self.array,self.boolean).libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("last",self.array,"TYPE").libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("first",self.array,"TYPE").libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("add",self.array.of(ClassParm("TYPE")),ClassParm("TYPE")).libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("addAll",self.array.of(ClassParm("TYPE")),self.array.of(ClassParm("TYPE"))).libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("removeAt",self.array.of(ClassParm("TYPE")),self.integer).libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("removeAll",self.array.of(ClassParm("TYPE")),self.array.of(ClassParm("TYPE"))).libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("withoutFirst",self.array.of(ClassParm("TYPE")),self.array.of(ClassParm("TYPE"))).libraryMethod)
        arrays.addSymbol(IntrinsicMethodInstance("withoutLast",self.array.of(ClassParm("TYPE")),self.array.of(ClassParm("TYPE"))).libraryMethod)
        let strings = SystemSymbolGroup(label: "Strings")
        self.addSymbol(strings)
        strings.addSymbol(IntrinsicMethodInstance("separatedBy",self.string,self.string,self.array.of(self.string)).libraryMethod)
        strings.addSymbol(IntrinsicMethodInstance("joinedWith",self.array.of(self.string),self.string,self.string).libraryMethod)
        strings.addSymbol(IntrinsicMethodInstance("contains",self.string,self.string,self.boolean).libraryMethod)
        strings.addSymbol(IntrinsicMethodInstance("hasPrefix",self.string,self.string,self.boolean).libraryMethod)
        strings.addSymbol(IntrinsicMethodInstance("hasSuffix",self.string,self.string,self.boolean).libraryMethod)
        strings.addSymbol(IntrinsicMethodInstance("prefixedWith",self.string,self.string,self.string).libraryMethod)
        strings.addSymbol(IntrinsicMethodInstance("suffixedWith",self.string,self.string,self.boolean).libraryMethod)
        }
    ///
    ///
