...
public class CustomView: NSView
    ...
    public func addActionButton(atEdge: ViewEdge,key: String,image: NSImage,toolTip: String,target: Any,action: Selector)
        ...
        button.insets = NSEdgeInsets(top: 1, left: 1, bottom: 1, right: 1)
        ...
        if atEdge == .left
            {
            button.leadingAnchor.constraint(equalTo: self.nextLeftAnchor,constant: 0).isActive = true
            self.nextLeftAnchor = button.trailingAnchor
            }
        else
            {
            button.trailingAnchor.constraint(equalTo: self.nextRightAnchor,constant: 0).isActive = true
